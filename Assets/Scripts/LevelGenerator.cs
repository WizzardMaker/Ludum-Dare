...
public class LevelGenerator : MonoBehaviour {
	...
	void CreateGroups(int minGroups, int maxGroups)
	{
		...
		{
			groups[i] = new Vector3(Random.Range(-xFloorSize / 2 + 25, xFloorSize / 2 - 25), 2.25f, Random.Range(-xFloorSize / 2 + 25, xFloorSize / 2 - 25));
		}
		...
		for (int group = 0; group < groups.Length; group++)
		{
			...
			for (int groupMemberA = 0; groupMemberA < members.Length; groupMemberA++)
			{
				for (int groupMemberB = 0; groupMemberB < members.Length; groupMemberB++)
				{
					...
					if (Vector3.Distance(members[groupMemberA].transform.position, members[groupMemberB].transform.position) <= 3)
					{
						members[groupMemberB].transform.position += (members[groupMemberA].transform.position - members[groupMemberB].transform.position) * -2;
					}
				}
			}
		}
